...
class PluginsLoader {
  ...
  checkDisable(p) {
    ...
    const defaultCfg = cfg.getConfig('group').default
    ...
    if (
      ...
      if(defaultCfg.disable?.length && defaultCfg.disable.includes(p.name)) return false
      if(defaultCfg.enable?.length) enableList = enableList.concat(defaultCfg.enable)
    }
    if (enableList.length && !enableList.includes(p.name)) return false
    return true
  }
