...
export default class Runtime {
  ...
  async render(plugin, path, data = {}, cfg = {}) {
    ...
    // 自动计算pluResPath、miaoResPath、layoutPath
    ...
    let _tpl_path = process.cwd() + "/plugins/miao-plugin/resources/common/tpl/"
    if(!fs.existsSync(`./plugins/miao-plugin`)) {
      miaoResPath = `../../../${lodash.repeat("../", paths.length)}resources/miao-plugin-resources/`
      layoutPath = process.cwd() + "/resources/miao-plugin-resources/common/layout/"
      _tpl_path = process.cwd() + "/resources/miao-plugin-resources/common/tpl/"
    }
    // 渲染data
    data = {
      sys: {
      ...
      /** miao 相关参数 **/
      copyright: `Created By Elia-Yunzai<span class="version">${Version?.yunzai || "Unknown"}</span> `,
